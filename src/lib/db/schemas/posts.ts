...
export const postsTable = sqliteTable('posts', {
    ...
    postAttachments: text('post_attachments', { mode: 'json' })
        .$type<GroupFeedPostAttachment[]>()
        .$default(() => sql`[]`),
    phoneNumbers: text('phone_numbers', { mode: 'json' })
        .$type<{ number: string; forWhatsApp: boolean; forPhoneCall: boolean; name?: string }[]>()
        .$default(() => sql`[]`),
    ...
    updatedAt: real('updated_at')
        .notNull()
        .default(sql`(strftime('%s', 'now'))`),
    creationTime: real('post_creation_time')
        .notNull()
        .default(sql`(strftime('%s', 'now'))`),
    processingStatus: text('processing_status', {
        enum: Object.values(PostProcessingStatus) as [PostProcessingStatus, ...PostProcessingStatus[]],
    })
        .notNull()
...
export const PostToPostTable = sqliteTable(
    'post_to_post',
    {
        parentPostId: text('parent_post_id')
            .notNull()
            .references(() => postsTable.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
        childPostId: text('child_post_id')
            .notNull()
            .references(() => postsTable.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
    },
    (table) => [primaryKey({ columns: [table.childPostId, table.parentPostId] })],
);
