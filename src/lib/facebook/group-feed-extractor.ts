...
async function extractFromNode(node: any): Promise<GroupFeedPost> {
	...
	const allAttechments = get(
		node,
		"comet_sections.content.story.attachments[0].styles.attachment.all_subattachments.nodes"
	) || get(
		node,
		"attachments[0].styles.attachment.style_infos[0].fb_shorts_story.attachments"
	) || [];
	if (allAttechments.length === 0) {
		const singleAttachment = get(
			node,
	...
	const postUrl = get(
		node,
		"comet_sections.feedback.story.story_ufi_container.story.url"
	) || get(
		node,
		"comet_sections.timestamp.story.url"
	);
	const publisherName = get(
		...
		node,
		"comet_sections.content.story.actors[0].id"
	);
	const publisherUrl = get(
		node,
		"actors[0].url"
	) || get(
		node,
		"comet_sections.content.story.actors[0].url"
	);
	const publisherId = get(
		node,
		"actors[0].id"
	) || get(
		node,
	...
	if (type === "Photo") {
		return {
			...
			url:
				...
				get(
					attachment,
					"media.comet_photo_attachment_resolution_renderer.image.uri"
				),
		};
	} else if (type === "Video") {
		const urls = get(
