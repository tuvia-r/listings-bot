    ...
    for (const filter of filters) {
        ...
        if (Array.isArray(value)) {
            ...
            if (!value.some((v) => isEqual(objValue, v))) {
                isValid = false;
                missmatchedFilters.push(`"${String(key)}" does not match any of the provided values: expected one of (${value.map(v => `"${v}"`).join(', ')}), got "${objValue}"`);
            }
        } else {
            // If value is a single value, check for equality
            if (!isEqual(objValue, value)) {
                isValid = false;
                missmatchedFilters.push(`"${String(key)}" does not match the expected value: expected "${value}", got "${objValue}"`);
            }
        }
    }
    return {
