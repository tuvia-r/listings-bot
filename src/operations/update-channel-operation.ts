...
export async function updateChannelWithNewPosts() {
    try {
        ...
        for (const post of unprocessedPosts) {
            try {
                ...
                if (success) {
                    ...
                    await Promise.all(post.childPosts.map((child) => markPostAsProcessed(child.childPostId)) || []);

                    logger.info(`Post ${post.postId} successfully processed and sent to Telegram`);
                    processedCount++;
                    ...
                    logger.error(`Failed to send post ${post.postId} to Telegram`);
                }
            } catch (error) {
        ...
        logger.info(`Successfully processed ${processedCount} out of ${unprocessedPosts.length} posts`);
